...
### Stage 0: 系统就绪与连接验证
在正式扫描媒体库之前，系统提供一键验证基础设施状态：
- **OpenSubtitles 账户联通**：验证账号密码有效性，确保后续下载字幕顺畅。
- **Translate API 连通测试**：检查配置的大模型（如 Gemini / Mistral / Groq 等）API Token 是否正常工作，保障高质量字幕翻译服务的可用性。

### Stage 1: 媒体库扫描
系统首先扫描媒体库目录，识别影片并解析元数据。
- **视频为王**：不再依赖容易缺失的 NFO 文件。系统会直接地毯式扫描整个媒体库，将所有大于 **100MB** 的有效视频文件（如 mkv/mp4 等）统统认定为一部独立影片，彻底解决同一目录下存在多版本电影（如导演剪辑版和剧场版共存）导致被忽略的问题。
- **元数据双轨绑定**：找到视频后，尝试读取周边的 `.nfo` 文件提取精准的标题、年份、**IMDB ID** 或 **TMDB ID**；即使缺少其中一个，系统也会自适应降级使用另一个进行精确匹配。如果实在没有 NFO，则会使用视频文件名作为默认片名，保证任务顺利推进。
- **状态持久化**：分析结果独立保存在该视频同目录下的 `[视频名].json` 中，互不干扰。
...
### Stage 4: 媒体语言鉴定 (STT Scan)
在处理字幕前，系统必须先准确摸清库里所有电影的真实发音与内置字幕情况。
- **音轨识别**：调用 FFmpeg 从视频中提取多个 15 秒片段（备选采样点 `[25%, 50%, 75%, 15%, 85%, 35%, 65%]`），并通过大模型 API（如 Groq 的 Whisper 模型）识别音轨语言。
- **内置字幕检测（全面兼容文本与图像字幕）**：从视频流中提取内置字幕轨道。对于文本格式字幕（如 SRT/ASS/VTT），抽样提取文本并统计 CJK 字符数量进行判断；对于无法直接提取文本的图像格式字幕（如蓝光原盘 PGS 或 DVD VobSub），则智能读取其轨道元数据的 `chi` 或 `zho` 语言标签进行判定。纯本地处理，无需外部 API。
- **宁缺毫滥的严格模式**：一旦识别到 3 个有效语音片段即见好就收。如果所有片段均为纯音乐或无声导致无法识别，**系统将抛出异常并中止任务**，强制要求人工介入。
- **监控大屏与管理页面**：提供独立的 Web 监控中心，可实时查看大盘进度和处理日志。并且内置专属的电影管理页，支持按年份、片名、是否含内置中文字幕、是否为中文发音等多个维度进行精准排序与筛选。
- **持久化档案**：分析结果保存在影片目录的 `[视频名].json` 中。

### Stage 5: 字幕补全（含同步检测与错版甄别）
只针对确认需要中文字幕的外语片执行操作：
- **OpenSubtitles 极速双轨匹配**：底层完全兼容 IMDB ID 与 TMDB ID。无论 NFO 中提供哪种 ID，都能精准直达对应字幕，从根本上解决部分老电影或亚洲电影因缺失 IMDB 数据而被无情跳过的痛点。
...
2. **每次刷新即“全盘重扫”**：
   ...
   - 脚本顺着配置的媒体库路径，实时且暴力地去扫物理硬盘上每一个大于 100MB 的视频文件，并嗅探它周边的字幕情况
...
- **subtitle_tracks.has_internal_chinese_sub**: 是否有内置中文字幕（由 Stage 4 内置字幕检测填充，null表示未检测，false表示无，true表示有）
