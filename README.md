...
- **元数据绑定**：找到视频后，就近尝试读取同名或目录下的 `.nfo` 文件来提取精准的标题、年份与 IMDB ID；如果实在没有 NFO，则会使用视频文件名作为默认片名，保证任务顺利推进。
...
- **音轨识别**：调用 FFmpeg 从视频中提取多个 15 秒片段（备选采样点 `[25%, 50%, 75%, 15%, 85%, 35%, 65%]`），并通过 Groq 提供的 Whisper 模型识别音轨语言。
- **内置字幕检测**：从视频流中提取内置字幕文本（0〆10〆20〆30 分钟四点采样），统计 CJK 字符数量判断是否为中文字幕。纯本地处理，无需 API。
- **宁缺毫滥的严格模式**：一旦识别到 3 个有效语音片段即见好就收。如果所有片段均为纯音乐或无声导致无法识别，**系统将抛出异常并中止任务**，强制要求人工介入。
- **监控大屏**：提供独立的 Web 监控中心，可实时查看大盘进度和处理日志。
